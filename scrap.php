...
class Document extends Model
{
	...
	public static public static function collectionName()
	{
	 ...
	 */
	public static function getPrimaryKey($value = null)
	{
		if($value === null){
	...
	public function insert($runValidation = true, $attributes = null)
	{
		...
		if($this->onBeforeSave()){
			...
			if($response = $this->getCollection()->insert($attributes, $this->getDb()->getDefaultWriteConcern())){
				$this->onAfterSave();
	...
	public function delete(){
		...
		if($this->onBeforeDelete()){
			$response = static::deleteAll($this->getPrimaryKey());
			$this->afterDelete();
			return $response;
		}else{
			return false;
    ...
    {
    	if(!is_array($query)){
    		$query = array(static::primaryKey() => static::getPrimaryKey($query));
    	}
    	$cursor = new Cursor(array(
    		'select' => $fields,
    		'from' => get_called_class(),
    ...
    public static function updateAll($query, $attributes, $options = array())
    {
    	...
    	return static::getCollection()->update($query, $attributes, array_merge(array('multiple' => true, $options)));
    }
    ...
    public static function saveAllCounters($query, $counters, $options = array())
    {
    	...
    	return static::getCollection()->update($query, array('$set' => $counters), array_merge(array('multiple' => true, $options)));
    }
    ...
    public static function deleteAll($query, $options = array())
    {
    	...
    	return static::getCollection()->remove($query, array_merge(array('multiple' => true, $options)));
    }    
