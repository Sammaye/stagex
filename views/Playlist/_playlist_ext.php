...
<?php }else{ ?>
	<div class='playlist_item <?php echo $extra_classes ?>' data-id='<?php echo isset($item) ? strval($item->_id) : strval($model->_id) ?>'
		...
		<div class='playlist_item_right no_margin'>
			...
			<?php if($model->author): ?>
				<div class='compiled_user'>Compiled by <a href="<?php echo glue::url()->create('/user/view', array('id' => strval($model->author->_id))) ?>"><?php echo $model->author->getUsername() ?></a></div>
			<?php else: ?>
				<div class='compiled_user'>Compiled by <a href="<?php echo glue::url()->create('/user/view') ?>">[User Deleted]</a></div>
			<?php endif; ?>
		</div>
		<div class='clearer'></div>
	</div>
