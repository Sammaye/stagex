...
<div class='video <?php echo isset($extra_classes) ? $extra_classes : '' ?>' data-id='<?php echo $model ? strval($model->_id) : '' ?>' data-ts='<?php echo $last_ts ?>' style='position:relative;'>
	...
	<div class='info'>
		...
		<?php if($model->author): ?>
			<div class='uploader'>
				Uploaded by <a href="<?php echo glue::http()->url('/user/view', array('id' => $model->author->_id)) ?>"><?php echo $model->author->getUsername() ?></a> <span class='divider'>|</span> <?php echo $model->views ?> views
			</div>
		<?php endif;
